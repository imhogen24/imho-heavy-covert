...
import RadialGradient from "../shared/radial-gradient";
    ...
    <div className="flex flex-col justify-center border-t muted-border overflow-x-hidden items-center relative h-full w-full">
      <div className="flex w-full h-52 border-b muted-border border-gradient-to-r from-gray-950 via-gray-500 to-gray-950 dark:from-white/5 dark:via-white/20 dark:to-white/5">
        ...
        <div className="flex flex-col items-center  m-auto gap-[16px] p-2">
          <h4
            className={cn(
              "text-center scroll-m-20 font-semibold tracking-tight text-2xl md:text-3xl lg:text-4xl",
              gradientText,
            )}
          >
            Our Impact
          </h4>
          <p className="text-center text-muted-foreground text-md md:text-xl max-w-sm md:max-w-2xl">
            IMHO&apos;s Distinct Impact: Advancing the UN SDGs and African
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 w-full h-full relative">
        {IMPACT_TEXT.map((impact, index) => (
          <div
            ...
            className={cn(
              "relative ",
              impact.footer && " md:border-r muted-border",
            )}
          >
            <div
      ...
      <div className=" relative flex flex-row items-center justify-center border-t muted-border w-full h-full">
        {SDG_GOALS.map((goal) => (
          <div
            key={goal.id}
            className="flex flex-row justify-evenly items-center w-full h-[199px] "
          >
            ...
            <Image
              ...
              src={goal.image}
              alt={`SDG ${goal.id}`}
              className="size-[80px] dark:hidden block  lg:size-[120px]"
            />
            ...
            <Image
              ...
              src={goal.imageLight}
              alt={`SDG ${goal.id}`}
              className="size-[80px] hidden dark:block lg:size-[120px]"
            />
          </div>
        ))}
      </div>

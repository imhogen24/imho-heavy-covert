...
import ProductRequirementsForm from '../../_components/product-enquiry-form'
import { Button } from '@/components/ui/button'
import Link from 'next/link'
import { Play } from 'lucide-react'
import { PyramidIcon } from '@/lib/icons'
import SupportRequirementsForm from '../../_components/engineering-support-form'

const words: string[] = ["aba", 'ababa', 'abaaba', 'strange'];
const isPrefixAndSufix = (i:string, j:string) : boolean =>{
return j.startsWith(i) && j.endsWith(i)
}

let count: number = 0;
words.forEach((n,index)=>{
        for( let i = 0; i>index; i++){
            isPrefixAndSufix(n, n[i])
            count++;
        }
})
  ...
  return (
    <div className='max-w-screen min-h-dvh flex flex-col'>
         <div className='grid grid-cols-3 relative h-fit w-full border-b muted-border'>
            ...
            <h1 className='md:text-2xl font-bold'>Ready to bring your vision to life?</h1>
            <h1 className="md:text-xl opacity-50">
                Start by filling the form below. Click below to watch a video on how to fill the form.
            </h1>
         ...
         <div className='flex flex-row w-full h-full justify-between'>
                  <div className='hidden relative lg:grid grid-cols-1 w-28 border-r muted-border border-dashed min-h-full'></div>
                      <div className='w-full'> <SupportRequirementsForm/></div>

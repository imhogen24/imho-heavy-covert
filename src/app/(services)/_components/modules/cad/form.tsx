...
import { cadFormAction } from "@/actions";
...
import { CadSchema, type CadFormData } from "@/lib/schemas/cad/z";
...
export function CadForm() {
  ...
  async function onSubmit(values: CadFormData) {
    ...
    if (values.documentationTypes) {
      formData.append(
        "documentationTypes",
        JSON.stringify(values.documentationTypes)
      );
    }
    ...
    if (values.otherDocumentationTypes) {
      formData.append(
        "otherDocumentationTypes",
        values.otherDocumentationTypes
      );
    }
    ...
    if (values.additionalDesignFeatures) {
      formData.append(
        "additionalDesignFeatures",
        values.additionalDesignFeatures
      );
    }
    ...
    formData.append(
      ...
      (values.requirePeriodicDrafts ?? false).toString()
    );

    if (values.additionalServices && values.additionalServices.length > 0) {
      formData.append(
        "additionalServices",
        JSON.stringify(values.additionalServices)
      );
    }
    ...
    if (values.fileAttachments && values.fileAttachments.length > 0) {
      formData.append(
        "fileAttachments",
        JSON.stringify(values.fileAttachments)
      );
    }
      ...
      <Form {...form}>
        ...
        >
          ...
          <FormSection label="Documentation Requirements">
            ...
            <SectionChild
              label="OTHER DOCUMENTATION TYPES"
              className="md:grid-cols-1"
            >
              <FormField
                control={form.control}
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Specify other documentation types if applicable
                    </FormLabel>
                    <FormControl>
                      <Textarea
            ...
            <SectionChild label="OTHER FILE FORMATS" className="md:grid-cols-1">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Specify other file formats if applicable
                    </FormLabel>
                    <FormControl>
                      <Textarea

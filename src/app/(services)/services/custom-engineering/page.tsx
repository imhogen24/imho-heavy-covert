import React from "react";
import { FactoryIcon } from "@phosphor-icons/react/dist/ssr/Factory";
import { CustomEngineeringForm } from "../../_components/modules/custom-engineering/form";
import ServiceHero from "../../_components/service-hero";

const CustomEngineeringPage = () => {
  return (
    <div className="max-w-screen min-h-dvh flex flex-col">
      <div className="grid grid-cols-3 relative h-fit w-full border-b muted-border">
        <div className="h-60 w-full border-r border-dashed muted-border"></div>
        <div className="h-60 w-full border-r border-dashed muted-border"></div>
        <div className="flex justify-center items-center h-60 w-full bg-neutral-100 dark:bg-neutral-900">
          <FactoryIcon size={96} weight="thin" className="dark:opacity-25" />
        </div>

        <ServiceHero
          title="Custom Engineering & Factory Solutions"
          subtitle="Master intake for product and process engineering. Define your system's inputs, transformation, outputs, and constraints, and we will turn them into a manufacturable design or a complete processing line."
        />
      </div>
      <div className="flex flex-row w-full h-full justify-between">
        <div className="hidden relative lg:grid grid-cols-1 w-28 border-r muted-border border-dashed min-h-full"></div>
        <div className="w-full">
          <CustomEngineeringForm />
        </div>
        <div className="hidden relative lg:grid grid-cols-1 w-28 border-l muted-border border-dashed min-h-full"></div>
      </div>
    </div>
  );
};

export default CustomEngineeringPage;

...
type Pattern = "corner-br" | "corner-bl" | "corner-tr" | "corner-tl" | "edges" | "center" | "cursor"
...
export const PixelBackground = ({
  ...
  const stateRef = useRef<AnimState>({
    cols: 0, rows: 0, dpr: 1,
    displayed: null, revealLevel: null, targets: null, delays: null,
    flickerTargets: null, flickerTimers: null,
    animating: false, hovering: false,
    animFrameId: null, lastTime: 0,
  })
  ...
  const setupGrid = useCallback(() => {
    ...
    s.cols = cols; s.rows = rows; s.dpr = dpr
  ...
  const computeDelays = useCallback((pat: Pattern, enterCol: number, enterRow: number) => {
    ...
    for (let c = 0; c < cols; c++) {
      for (let r = 0; r < rows; r++) {
        const idx = c * rows + r
        let d = 0
        if      (pat === "corner-br") d = Math.hypot(c - (cols - 1), r - (rows - 1))
        ...
        else if (pat === "edges")     d = Math.min(c, cols - 1 - c, r, rows - 1 - r)
  ...
  const tick = useCallback((time: number) => {
    const s = stateRef.current
    if (
      !s.displayed || !s.revealLevel || !s.targets || !s.delays ||
      !s.flickerTargets || !s.flickerTimers
    ) return

    const dt = Math.min((time - s.lastTime) / 1000, 0.05)
    s.lastTime = time

    const { displayed, revealLevel, targets, delays, flickerTargets, flickerTimers } = s
    let anyActive = false
    const ls = 6 * speed
